...
- Buttons (`pin_fuente`, `pin_brillo`, `INPUT_PULLUP`) trigger on LOW with 200 ms `delay` debounce + immediate `guardar_config()`. Canonical default pins are 18/19 in both code and `data/config.json` — devices with an old LittleFS `/config.json` (16/17) keep their stored copy until `uploadfs`/reformat.
